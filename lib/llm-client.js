// OpenAI-compatible API client for text summarization and MiMo TTS.
//
 ...
 * @returns {{ complete: (opts: { system?: string, prompt: string, config?: object, signal?: AbortSignal }) => Promise<{ text: string | null, error?: string }>, synthesize: (opts: { text: string, style?: string, voice?: string, config?: object, signal?: AbortSignal }) => Promise<{ audioData: string | null, error?: string }> }}
 */
export function createClient(kv, pluginOptions) {
   ...
   */
  async function complete({ system, prompt, config: overrides, signal }) {
   ...
   * @param {string} [opts.style]    - Natural-language voice style instruction
   ...
   */
  async function synthesize({ text, style, voice, config: overrides, signal }) {
    const cfg = { ...getTTSConfig(), ...overrides };
    const messages = [];
    if (style?.trim()) messages.push({ role: "user", content: style.trim() });
