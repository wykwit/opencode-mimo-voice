...
const DEFAULT_TTS_STYLE =
  "bright, lively anime heroine voice; quick pace; expressive rhythm; cheerful and confident tone; playful but still clear and useful";
...
Delivery style:
- Target MiMo TTS style: bright, lively anime heroine voice; quick pace; expressive rhythm; cheerful and confident tone; playful but still clear and useful
- Prefer short, punchy sentences that sound natural at a fast speaking pace
- Add light playful flavor when it fits: tiny reactions, spirited transitions, or a quick wink of personality are welcome
- Keep technical content accurate. Do not invent extra facts, filenames, commands, results, or claims
- Do not output XML/style tags; the playback layer adds the MiMo style tag separately
...
Delivery style:
- Target MiMo TTS style: bright, lively anime heroine voice; quick pace; expressive rhythm; cheerful and confident tone; playful but still clear and useful
- Preserve the user's content, but make the phrasing smooth for fast spoken delivery
- Add light playful flavor only where it does not change meaning: tiny reactions or spirited transitions are fine
- Keep technical content accurate. Do not invent extra facts, filenames, commands, results, or claims
- Do not output XML/style tags; the playback layer adds the MiMo style tag separately
...
export function registerTTS(api, kv, complete, synthesize, prompts, options = {}) {
  ...
  function applyTTSStyle(text) {
    const style = getTTSStyle().trim();
    if (!style || /^\s*<style>/i.test(text)) return text;
